...
// +----------------------------------------------------------------------
const SENTCMS_VERSION    = '2.0.20160122';
const SENT_ADDON_PATH = './Addons/';
...
function authcode($string, $operation = 'DECODE', $key = '', $expiry = 0) {
    ...
    $uc_key = C('AUTH_KEY') ? C('AUTH_KEY') : AUTH_KEY;
...
function get_cover($cover_id, $field = null){
    if(empty($cover_id)){
        return __ROOT__.'/Public/images/default.jpg';
    }
    $picture = M('Picture')->where(array('status'=>1))->getById($cover_id);
    if($field == 'path'){
        if(!empty($picture['url'])){
            $picture['path'] = $picture['url'] ? $picture['url'] : __ROOT__.'/Public/images/default.jpg';
        }else{
            $picture['path'] = $picture['path'] ? __ROOT__.$picture['path'] : __ROOT__.'/Public/images/default.jpg';
        }
    }
...
function avatar($uid, $size = 'middle') {
    ...
    $file = '/Uploads/Avatar/'.$dir.'avatar_'.$size.'.jpg';
    if (!file_exists('.'.$file)) {
        $file = '/Public/images/default_avatar_'.$size.'.jpg';
    }
    return $file;
}
 ...
 * @author 麦当苗儿 <zuojiazi@vip.qq.com>
 */
function get_list_count($category, $status = 1,$child){
    static $count;
    ...
    if(!isset($count[$category])){
        $count[$category] = D('Document')->listCount($categorys, $status);
    }
...
function sendMsg($mobile,$content){
    $api = C('WEB_SITE_MSG_API');
    $user = C('WEB_SITE_MSG_USER');
    $key = C('WEB_SITE_MSG_KEY');
    $url = $api."?Uid=".$user."&Key=".$key."&smsMob=".$mobile."&smsText=".$content;
    ...
    $data = array(
        'code'  => $file_contents,
        'msg'   => $res[$file_contents],
    );
    return $data;
}

//去除数组中重复的数组
function list_unique($array){
   $out = array();
...
function get_ip_address($ip){
    ...
    $header = array(
        'apikey: efd4b377da9baf10cc3670e424c66780',
    );
    // 添加apikey到header
    ...
    if ($res->errMsg == 'success') {
        $return = $res->retData;
        return $return->country.$return->province.$return->city.$return->district;
    }else{
        return "未知地址";
    }
}

function get_list_filed($list_grid){
    $grids  =   preg_split('/[;\r\n]+/s', trim($list_grid));
