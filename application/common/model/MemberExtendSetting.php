...
class MemberExtendSetting extends Model {
    ...
    protected $_validate = array(
        ...
        array('extend_id', 'require', '未选择操作的分类', self::MUST_VALIDATE, 'regex', self::MODEL_BOTH),
    );
    ...
    public function update($data = null, $create = true){
        ...
        //删除字段缓存文件
        // $model_name = M('Model')->field('name')->find($data['extend_id']);
    ...
    protected function checkName(){
        ...
        $id = I('post.id');
        $map = array('name'=>$name, 'extend_id'=>$extend_id);
        if(!empty($id)){
            $map['id'] = array('neq', $id);
        }
    ...
    /**
     * 检查当前表是否存在
     * @param intger $extend_id 模型id
     * @return intger 是否存在
     * @author huajie <banhuajie@163.com>
     */
    protected function checkTableExist($extend_id){
        $Model = M('Model');
        //当前操作的表
        $model = $Model->where(array('id'=>$extend_id))->field('name,extend')->find();
...
sql;
    ...
    protected function addField($field){
        //检查表是否存在
        $table_exist = $this->checkTableExist($field['extend_id']);
...
sql;
            ...
            //新建表时是否默认新增“id主键”字段
            $model_info = M('Model')->field('engine_type,need_pk')->getById($field['extend_id']);
